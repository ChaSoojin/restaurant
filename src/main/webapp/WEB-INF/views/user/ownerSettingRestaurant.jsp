...
<table border="solid 1px" style="border-collapse: collapse; width: 800px; height: 400px; margin:auto; text-align:center;">
    ...
    <c:set var="restaurant_id" value="${shop.getRestaurant_id()}"/>
    <c:out value="user_id: ${user_id}"/>
    <c:out value="restaurant_id: ${restaurant_id}"/>

    <tr>
        <td>식당명</td>
        <td colspan="5">${shop.getRestaurant_name() }</td>
    </tr>
    <tr>
        <td>식당전화번호</td>
        <td colspan="5">${shop.getPhone()}</td>
    </tr>
    <tr>
        <td>식당주소</td>
        <td colspan="5">${shop.getAddress()}</td>
    </tr>
</table>
